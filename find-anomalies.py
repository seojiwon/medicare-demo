...
specialty_file = path+"specialty.txt"
...
# This file graph.txt can be generated by running run1.sh, which preprocess the Medicare data set and 
...
# npi-cpt-code.txt: NPI <tab> NPI specialty <tab> CPT code <tab> CPT code frequency
# NPI table stores the information (NPI, specialty, CPT code, frequency)
npi_cpt_file = path+"npi-cpt-code.txt"
...
# hcpcs_code.txt: CPT code <tab> description
# Specialty table maps specialty id to its description.
cpt_file = path+"hcpcs-code.txt"
...
for i in range(len(specialties)):
    ...
    # Specialties occuring more than threshold(20) are also considered false-positives.
    specialtyCount={}
