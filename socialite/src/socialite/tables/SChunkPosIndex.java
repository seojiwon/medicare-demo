...
public final class SChunkPosIndex {
	volatile Object index;

	public SChunkPosIndex() {}

	public void clear() {
		if (index==null) return;

        if (index instanceof ConcurrentIntOrderedListMap) iIndex().clear();
    ...
    public void addAtomic(int key, ChunkPos pos) {
        if (pos==null)
            throw new NullPointerException();
        for (;;) {
            Object val = iIndex().get(key);
            if (val == null) {
                boolean success = iIndex().putAtomicIfAbsent(key, pos) == null;
                if (success)
                    return;
            } else if (val instanceof ChunkPos) {
                IndexPosList<ChunkPos> posList = new IndexPosList<ChunkPos>();
                posList.add((ChunkPos)val);
                posList.add(pos);
                boolean success = iIndex().replace(key, val, posList);
                if (success)
                    return;
            } else {
                IndexPosList<ChunkPos> posList = (IndexPosList<ChunkPos>) val;
    ...
    public void iterateFrom(int from, boolean fromInclusive, ObjectVisitor v) {
        ConcurrentIntOrderedListMap.IntObjectIterator iter = iIndex().iteratorFrom(from, fromInclusive);
        iterate(iter, v);
    }
    public void iterateTo(int to, boolean toInclusive, ObjectVisitor v) {
        ConcurrentIntOrderedListMap.IntObjectIterator iter = iIndex().iteratorTo(to, toInclusive);
        iterate(iter, v);
    }
    public void iterateFromTo(int from, boolean fromInclusive, int to, boolean toInclusive, ObjectVisitor v) {
        ConcurrentIntOrderedListMap.IntObjectIterator iter = iIndex().iterator(from, to, fromInclusive, toInclusive);
        iterate(iter, v);
    }
    ...
    static void testObjIndex() {
        SChunkPosIndex index = new SChunkPosIndex();
        for (int i=0; i<10; i++) {
...
class IndexPosList<V> {
    ...
    static final AtomicReferenceFieldUpdater<IndexPosList, Node> tailUpdater =
            AtomicReferenceFieldUpdater.newUpdater(IndexPosList.class, Node.class, "tail");
