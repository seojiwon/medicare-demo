...
public class VisitorCodeGen {
	...
	void insertIterateCodeInRun(ST run, String tableVar) {
        ...
        if (updateFromRemoteHeadT()) {
        	String lock = lockMapVar()+".getLock("+headT.id()+","+firstTableIdx()+")";
            invokeIter = tableVar+".iterate_slice("+lock+","+sliceMapVar()+","+headT.id()+","+firstTableIdx()+",this)";
	...
	boolean headTableLockNeeded() {
		...
		if (updateFromRemoteHeadT()) {
			// See FixedSizeTable.iterate_slice() and VisitorCodeGen.insertIterateCodeInRun()
			return false; 
		}
